...
CHttpHost::CHttpHost(unsigned short port) :
	port(port), running(false), CreateFileTaskFunction(NULL), epfd(0) {
}
void CHttpHost::Stop() {
	running = false;
}
void CHttpHost::Run() {
	...
	pthread_t workid[workThreadCount] = { 0 };
		...
		if (epollres == 0) {
			...
			for (int i = 0; i < epollres; i++) {
					...
					if (nowev.data.fd == listenfd) {
						for (;;) {
							...
							if (connfd == -1) {
								...
								res = epoll_ctl(epfd, EPOLL_CTL_ADD, connfd,
										&ev);
								if (res == 0) {
									...
									socketstates.insert(
											SocketStates::value_type(connfd,
													state));
									...
									writequeuemap.insert(
											WriteQueueMap::value_type(connfd,
													writequeue));
									writequeuemaplock.Unlock();
				...
				if (nowev.events & EPOLLOUT) {
					sokectstatesMutex.Lock();
					SocketStates::iterator nowsocketstate = socketstates.find(
							nowev.data.fd);
					if (nowsocketstate != socketstates.end()) {
...
void CHttpHost::writeSocketThread() {
  ...
  while (running) {
				  ...
				  while (writequeue->size() > 0) {
					  ...
					  if (writequeue->lastPos >= nowbuf->datalen) {
						  ...
						  int writeres = write(i->first,
								  nowbuf->Buffer() + writequeue->lastPos,
								  towrite);
						  if (writeres == -1) {
							  if (errno != EAGAIN) {
	  ...
	  for (std::set<int>::iterator i = fddead.begin(); i != fddead.end(); i++) {
		  closesocket(*i);
	  }
  }
}
	...
	while (running) {
		LPCBUFFER buffer;
		{
			...
			while (walker != readdataqueue.end()) {
				if (processingFile.end() == processingFile.find(
						(*walker)->exdata)) {
					buffer = *walker;
		...
		CIPtr<FileTask> fdtask;
		{
			...
			std::map<int, CIPtr<FileTask> >::iterator walker = taskpoll.find(
					buffer->exdata);
			if (walker != taskpoll.end()) {
				...
				taskpoll.insert(
						std::map<int, CIPtr<FileTask> >::value_type(
								buffer->exdata, fdtask));
			}
